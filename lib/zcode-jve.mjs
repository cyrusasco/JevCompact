...
export async function compactZcodeSession(idOrPrefix, opts = {}) {
  const { apply = false, keep = 6, threshold = 0.6, truncateHead = 300, policy = true, goal, force = false, minReduction = 0.05, maxStateTokens = 25000, maxRequestTokens = 30000, log = () => {} } = opts;
...
if (IS_MAIN) {
  ...
  if (!target) { console.error("usage: node zcode-jve.mjs <session-id|prefix|live> [--apply] [--keep=6] [--threshold=0.6] [--truncate-head=300] [--min-reduction=0.05] [--max-state-tokens=25000] [--max-request-tokens=30000] [--goal=...] [--no-policy] [--force]"); process.exit(2); }
