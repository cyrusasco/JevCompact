  ...
  for (const row of messageRows) {
    ...
    for (const p of partRowsOf.all(row.id)) {
      ...
      if (pd.type === "text") texts.push(pd.text ?? "");
        ...
        // judgement payload: output first; when absent, the metadata.display payload (the
        // console-family tools carry most of their payload there — both stay in the stored
        // row untouched); then error. Capped so no huge display blob ships unbounded to the
        // remote; the cap marker is excluded from the fact-count by construction.
        let jp = st.output ?? st.metadata?.display ?? st.error ?? (st.status === "error" ? "error" : "");
