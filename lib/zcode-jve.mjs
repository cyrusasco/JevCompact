...
export async function compactZcodeSession(idOrPrefix, opts = {}) {
  const { apply = false, keep = 6, threshold = 0.6, truncateHead = 300, policy = true, goal, force = false, minReduction = 0.05, maxStateTokens = 25000, maxRequestTokens = 30000, dedup = false, trimCarriers = false, bookkeeping = false, allowDerived = false, dbPath = DB_PATH, log = () => {} } = opts;
  ...
  try {
    ...
    if (session.id.includes("sess_subagent") || (session.parent_id && !allowDerived)) {
      return { ok: false, error: `refused: ${session.id} is a subagent/derived session — listing and compaction both exclude it (it serves only its parent)` + (session.parent_id && !allowDerived ? " (pass allowDerived:true to override explicitly)" : "") };
    }
...
function outcomeResolveSession(db, idOrPrefix, allowDerived = false) {
  const session = idOrPrefix === "live"
    ? db.prepare(`SELECT s.id, s.title, s.parent_id FROM session s
        WHERE (SELECT MAX(m.time_created) FROM message m WHERE m.session_id=s.id) IS NOT NULL
          AND instr(s.id, 'sess_subagent') = 0 AND (s.parent_id IS NULL OR s.parent_id = '')
  ...
  if (session.id.includes("sess_subagent")) return { refused: session.id };
  if (session.parent_id && !allowDerived) return { refused: session.id + " (derived session — pass allowDerived to override)" };
  return session;
}
...
export function planOutcomeTrimForSession(idOrPrefix, opts = {}) {
  const { topics, keepLast = 1, archiveFailures = false, allowDerived = false, dbPath = DB_PATH, log = () => {} } = opts;
  const db = new DatabaseSync(dbPath, { open: true, readOnly: true });
  try {
    const session = outcomeResolveSession(db, idOrPrefix, allowDerived);
    if (!session) return { ok: false, error: `no such session: ${idOrPrefix}` };
    ...
    const { plan, notes } = planOutcomeTrim({ rows, topics, opts: { keepLast, archiveFailures } });
...
export async function applyOutcomePlan({ planFile, plan: planIn, apply = false, dbPath = DB_PATH, skillPath = null, backupDir = path.join(os.homedir(), ".zcode", "backups"), log = () => {} } = {}) {
  ...
  try {
    ...
    // 1. verified backup (backupDir is the function parameter — isolated drills redirect it)
    fs.mkdirSync(backupDir, { recursive: true });
