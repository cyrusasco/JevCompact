...
/* ------------------------------------------------------------- actions */
// auto triage (the R1–R4 reference rule): when none of the three v1.1 boxes is ticked, the
// passes are derived from the measured shares of the very session — the user does not need
// to know the classes. R1 (ledger-dominant, snapshot >= 40 %) ⇒ all three passes (the
// bookkeeping pair carries the bulk there); R2 (console load >= 60 %) ⇒ bookkeeping only
// (dedup/trim measured 0 extra on console-type); R3/R4 ⇒ base policy alone.
function triagePasses(id) {
  const db = new DatabaseSync(DB_PATH, { open: true, readOnly: true });
  try {
    const mrows = db.prepare("SELECT id, data FROM message WHERE session_id = ? ORDER BY time_created, rowid").all(id);
    const prows = db.prepare("SELECT data FROM part WHERE message_id = ? ORDER BY (sequence IS NULL), sequence, rowid");
    let total = 0, snap = 0, rec = 0, pay = 0, textB = 0;
    for (const m of mrows) {
      total += Buffer.byteLength(m.data, "utf8");
      for (const p of prows.all(m.id)) {
        const b = Buffer.byteLength(p.data, "utf8"); total += b;
        let pd; try { pd = JSON.parse(p.data); } catch { continue; }
        if (pd?.type === "text") { textB += b; continue; }
        if (pd?.type !== "tool") continue;
        const st = pd.state ?? {}; const isC = /node_repl|computer-use|browser/i.test(String(pd.tool ?? ""));
        if (isC) rec += b;
        const rf = st.metadata?.readFileState;
        if (rf && typeof rf.content === "string") snap += Buffer.byteLength(rf.content, "utf8");
        if (isC) {
          const o = st.output, d = st.metadata?.display;
          pay += (typeof o === "string" ? Buffer.byteLength(o, "utf8") : 0) + (d == null ? 0 : (typeof d === "string" ? Buffer.byteLength(d, "utf8") : Buffer.byteLength(JSON.stringify(d), "utf8")));
        }
      }
    }
    const pct = (x) => (total ? (100 * x) / total : 0);
    if (pct(snap) >= 40) return { dedup: true, trimCarriers: true, bookkeeping: true, series: "R1" };
    if (pct(rec) >= 60) return { dedup: false, trimCarriers: false, bookkeeping: true, series: "R2" };
    if (pct(textB) >= 60) return { dedup: false, trimCarriers: false, bookkeeping: false, series: "R3" };
    return { dedup: false, trimCarriers: false, bookkeeping: false, series: "R4" };
  } finally { try { db.close(); } catch { /* closed */ } }
}
async function compactAction({ harness, id, dryRun, keep, threshold, truncateHead, goal, minReduction, maxStateTokens, maxRequestTokens, dedup, trimCarriers, bookkeeping }) {
  const logRing = [];
  if (!dedup && !trimCarriers && !bookkeeping) { const t = triagePasses(id); dedup = t.dedup; trimCarriers = t.trimCarriers; bookkeeping = t.bookkeeping; logRing.push("auto triage: " + t.series + " ⇒ " + ([dedup && "dedup", trimCarriers && "trim-carriers", bookkeeping && "bookkeeping"].filter(Boolean).join(" + ") || "base policy")); }
  if (harness === "zcode" || (harness === undefined && dbHasSession(id))) {
      const report = await compactZcodeSession(id, { apply: !dryRun, keep: Number(keep ?? 6), threshold: Number(threshold ?? 0.6), truncateHead: Number(truncateHead ?? 300), ...(minReduction ? { minReduction: Number(minReduction) } : {}), ...(maxStateTokens ? { maxStateTokens: Number(maxStateTokens) } : {}), ...(maxRequestTokens ? { maxRequestTokens: Number(maxRequestTokens) } : {}), ...(goal ? { goal } : {}), ...(dedup ? { dedup: true } : {}), ...(trimCarriers ? { trimCarriers: true } : {}), ...(bookkeeping ? { bookkeeping: true } : {}), log: (m) => logRing.push(m) });
...
<main>
  <div id="paneSessions">
    <div class="bar">
      ...
      <label class="muted" style="font-size:12px"><input type="checkbox" id="optDedup"> 去重</label>
      <label class="muted" style="font-size:12px"><input type="checkbox" id="optTrim"> 承載裁剪</label>
      <label class="muted" style="font-size:12px"><input type="checkbox" id="optBook"> 帳簿清掃</label>
      <span class="muted" style="font-size:11px">（唔撳＝自動：按該 session 嘅位元構成 triage R1–R4）</span>
...
function act(r, dry, btn) {
  ...
  fetch("/api/compact", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ harness: r.harness, id: r.id, dryRun: dry, limit: Number($("#limitIn").value) || 500000, dedup: $("#optDedup").checked, trimCarriers: $("#optTrim").checked, bookkeeping: $("#optBook").checked }) })
    ...
    .then(function (j) {
      ...
      if (j.ok && j.plan) {
        ...
        if (j.sql) { postLog("ok", "  sql: DELETE " + j.sql.deletes + " · UPDATE " + j.sql.updates + (j.sql.clears ? " · CLEAR " + j.sql.clears : "") + " · 文字零觸碰 " + j.sql.nonToolPartsTouched); }
        if (j.bookkeeping) { postLog("ok", "  bookkeeping: " + j.bookkeeping.files + " paths · 舊快照正文清除 " + j.bookkeeping.clearedRows + " 行 (" + j.bookkeeping.clearedBytes + " B)"); }
        if (j.ledger) { postLog("ok", "  決策帳: " + String(j.ledger.file).split(/[\\/]/).pop() + " — " + j.ledger.decisions + " 行, sha256 " + String(j.ledger.sha256 || "").slice(0, 12) + "…"); }
