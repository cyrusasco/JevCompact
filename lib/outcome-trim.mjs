...
export function planOutcomeTrim({ rows, topics, opts = {} }) {
  ...
  const tier2 = !!opts.archiveFailures; // user declaration: dead-end evidence may be archived too (recoverable from the verified backup)
  for (const r of inScope) {
    const head = String(r.output_text ?? "").replace(/\s+/g, " ").slice(0, 80);
    if (isCritical(r)) {
      if (tier2) { candidatesTier2.push({ part_id: r.part_id, bytes: r.bytes, replaced_by: "", override_of: "none", head, archived_evidence: true }); continue; }
      retained.push({ part_id: r.part_id, bytes: r.bytes, reason: r.status === "error" ? "failure_evidence" : "critical_evidence", head }); continue;
    }
    if ((r.output_text ?? "").length === 0) {
      if (tier2) { candidatesTier2.push({ part_id: r.part_id, bytes: r.bytes, replaced_by: "", override_of: "none", head, archived_evidence: true }); continue; }
      retained.push({ part_id: r.part_id, bytes: r.bytes, reason: "empty_result_unproven", head }); continue;
    }
  ...
  const plan = {
    ...
    candidates: candidates.concat(tier2 ? candidatesTier2.map((c) => ({ ...c, replaced_by: anchor.part_id })) : []).map((c) => ({ ...c, bytes_final_after_apply: 0 })),
    retained: retained,
    tier2: tier2 ? { archive_failures: true, declaration: "user-declared: failure/empty evidence inside the declared scope may be archived; recoverable from the verified backup (sealed set)" } : undefined,
